...
@section('main-content')
    <div class="card card-warning">
        ...
        <div class="card-body">
            <form action="{{ $offer->exists ? route('offer.update', $offer) : route('offer.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    @endif
                    <input type="text" class="form-control is-valid" name="title" id="title" placeholder="Nom ..." value="{{ $offer->exists ? $offer->title : old('title') }}">
                </div>
                <div class="form-group">
                    @if(isset($errors))
                    ...
                    @endif
                    <input type="url" class="form-control is-valid" name="url" id="url" placeholder="Url ..." value="{{ $offer->exists ? $offer->url : old('url') }}">
                </div>
                        ...
                        <div class="el-container-grid-column-document">
                            <div id="el-container-picture" style="width: 100%;">
                                <div class="el-container">
                                    <img src="{{ asset($offer->document->path) }}" alt="..." class="img-thumbnail">
                                    <button class="el-remove"
                                            hx-delete="{{ route('document.destroy', ['document' => $offer->document, 'isMultiple' => 0]) }}"
                ...
                <div class="row">
                    <div class="col-12">
                        <textarea id="summernote" name="description"> {{ $offer->description ?? old('description') }} </textarea>
                    </div>
                </div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const pond = FilePond.create(inputElement, {
                    ...
                    server: {
                        process: {
                            url: '{{ route("color.uploadDocument", $offer) }}', // Remplacez par l'URL de votre action Laravel pour l'upload
                            method: 'POST',
