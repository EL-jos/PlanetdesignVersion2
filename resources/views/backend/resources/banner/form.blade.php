...
@section('main-content')
    <div class="card card-warning">
        ...
        <div class="card-body">
            <form action="{{ $banner->exists ? route('banner.update', $banner) : route('banner.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                        ...
                        <div class="el-select2">
                            <select id="category_id" class="form-control el-select" name="category_id">
                                ...
                                @foreach($categories as $category)
                                    @if($banner->exists && $banner->category->id === $category->id)
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            <input type="url" class="form-control is-valid" name="url" id="url" placeholder="Url ..." value="{{ $banner->exists ? $banner->url : old('url') }}">
                        </div>
                    </div>
                </div>
                        ...
                        <div class="el-container-grid-column-document">
                            <div id="el-container-picture" style="width: 100%;">
                                <div class="el-container">
                                    <img src="{{ asset($banner->document->path) }}" alt="..." class="img-thumbnail">
                                    <button class="el-remove"
                                            hx-delete="{{ route('document.destroy', ['document' => $banner->document, 'isMultiple' => 0]) }}"
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const pond = FilePond.create(inputElement, {
                    ...
                    server: {
                        process: {
                            url: '{{ route("color.uploadDocument", $banner) }}', // Remplacez par l'URL de votre action Laravel pour l'upload
                            method: 'POST',
