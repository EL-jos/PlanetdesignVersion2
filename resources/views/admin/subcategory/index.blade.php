...
@section('main-content')
    ...
    <div class="d-flex gap-2">
        <a class="btn btn-primary" href="{{ route('subcategory.create') }}" role="button">Ajouter</a>
        <a class="btn btn-info" href="{{ route('subcategory.deleted') }}" role="button">Sous catégorie supprimée</a>
    </div>
                            ...
                            <tbody>
                                @foreach($subcategories as $subcategory)
                                    <tr data-subcategory-id="{{ $subcategory->id }}">
                                        <td>
                                            <img src="{{ asset($subcategory->image ? $subcategory->image->path : '') }}" class="avatar avatar-sm me-3 border-radius-lg" alt="user1">
                                        </td>
                                        <td>
                                            <h6 class="mb-0 text-sm">{{ $subcategory->name }}</h6>
                                        </td>
                                        <td class="align-middle text-center text-sm">
                                            <span class="badge badge-sm bg-gradient-success">{{ $subcategory->category->name }}</span>
                                        </td>
                                        <td class="align-middle text-center text-sm">
                                            <span class="badge badge-sm bg-gradient-success">{{ $subcategory->delete_at == null ? 'OnLine' : 'OnffLine' }}</span>
                                        </td>
                                        <td class="align-middle text-center">
                                            <span class="text-secondary text-xs font-weight-bold">{{ date('d/m/Y à H:m:s', strtotime($subcategory->updated_at)) }}</span>
                                        </td>
                                        <td class="align-middle">
                                            <div class="d-flex gap-2 w-100 justify-content-end">
                                                @if(!$subcategory->trashed())
                                                    <a href="{{route('subcategory.edit', $subcategory)}}" class="btn btn-primary" data-toggle="tooltip" data-original-title="Edit user">
                                                        Editer
                                                    </a>
                                                ...
                                                @else
                                                    <a href="{{ route('subcategory.restore', $subcategory) }}" class="btn btn-success">Restaurer</a>
                                                    <a href="{{ route('subcategory.remove', $subcategory) }}" class="btn btn-danger">Supprimer définitivement</a>
                                                @endif
                                            </div>
            ...
            var sortable = new Sortable(document.querySelector('table tbody'), {
                ...
                onEnd: function (event) {
                    ...
                    var itemsIds = Array.from(sortable.el.children).map(function (element) {
                        return element.getAttribute('data-subcategory-id');
                    });
                    ...
                    axiosInstance.post('{{ route('updateOrder.subcategory') }}', formData, {
                        headers: {
