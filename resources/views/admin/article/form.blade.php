            ...
            <div class="card my-4">
                ...
                <div class="card-body pb-2">
                    <form method="post" action="{{ $article->exists ? route('article.update', $article) : route('article.store', $article) }}" enctype="multipart/form-data">
                        ...
                        <div @class(["mb-3 input-group input-group-outline", "is-focused" => $article->exists])>
                            <label class="form-label">Priorité</label>
                            <input type="number" class="form-control" name="priority" value="{{ $article->priority }}" min="0" />
                        </div>
                        ...
                        <div class="mb-3">
                            <select id="subcategory_id" class="form-control" name="subcategory_id[]" multiple>
                                <option value="">Sous catégorie(s)</option>
                                @foreach($subcategories as $subcategory)
                                    @if($article->exists && $article->subcategories->contains($subcategory->id))
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <div class="mb-3">
                                <style>
                                    .ts-wrapper.form-control.single.plugin-remove_button{
                                        padding: 0;
                                    }
                                </style>
                                <select id="size_id" class="form-control" name="size_id[]" multiple>
                                    <option value="">Taille(s)</option>
                                    @foreach($sizes as $size)
                                        @if($article->exists && $article->sizes->contains($size->id))
                        ...
                        @if($article->exists)
                            <a href="javascript:;" data-url="{{ route('article.destroy', $article) }}" id="delete-link"  class="btn btn-danger">
                                Supprimer
                            </a>
                        @endif
                    </form>
...
@section('scripts')
    ...
    <script>
        ...
        new TomSelect('select#subcategory_id', {
